...
vanetza::asn1::Denm DenmMessage::buildDenm() const {
	...
	*mgmt.transmissionInterval = management.transmissionInterval.count();
	...
	loc.traces.list.array = static_cast<struct PathHistory**>(vanetza::asn1::allocate(sizeof(struct PathHistory*)));

	// Allocate and initialize one PathHistory entry
	loc.traces.list.array[0] = static_cast<struct PathHistory*>(vanetza::asn1::allocate(sizeof(struct PathHistory)));

	// Initialize the PathHistory
	PathHistory_t* path = loc.traces.list.array[0];
	path->list.count	= 0;
	path->list.size		= 0;
	path->list.array	= static_cast<struct PathPoint**>(vanetza::asn1::allocate(0));
...
void DenmMessage::fromUper(const std::vector<unsigned char>& data) {
	...
	rval = uper_decode_complete(nullptr, &asn_DEF_DENM, (void**)&denm, data.data(), data.size());
	...
	if (denm->header.messageID != ItsPduHeader__messageID_denm) {
		spdlog::error("Invalid message ID: {} (expected {})", denm->header.messageID, ItsPduHeader__messageID_denm);
		ASN_STRUCT_FREE(asn_DEF_DENM, denm);
	...
	try {
		...
		if (denm->denm.location) {
			...
			if (loc.eventSpeed) {
				...
				spdlog::debug("Decoded speed: {} m/s, confidence: {}", situation.eventSpeed, location.speedConfidence);
			}
			if (loc.eventPositionHeading) {
				...
				spdlog::debug(
				  "Decoded heading: {} degrees, confidence: {}", situation.eventHeading, location.headingConfidence);
			}
	...
	logMessage << "Management Container:\n"
			   ...
			   << "  Event Position: " << management.eventPosition.latitude << ", "
			   << management.eventPosition.longitude << ", " << management.eventPosition.altitude.altitudeValue << "\n"
