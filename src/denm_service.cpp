...
DenmService::DenmService(const std::string& amqp_url,
...
DenmMessage DenmService::createDenmFromJson(const crow::json::rvalue& j) {
	...
	denm.setEventPosition(j["latitude"].d(), j["longitude"].d(), j.has("altitude") ? j["altitude"].d() : 0.0);
...
void DenmService::start() {
	...
	if (receiver_mode_) {
		try {
			...
			amqp_receiver_ =
			  std::make_unique<receiver>(*amqp_container_, amqp_url_, amqp_receive_address_, "denm-receiver");
